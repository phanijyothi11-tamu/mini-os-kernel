...
void test_large_file(FileSystem* _file_system) {
	Console::puts("\n=== Running Bonus Option 2: Large File Test ===\n");

	assert(_file_system->CreateFile(42));
	File file(_file_system, 42);

	const int SIZE = 64 * 1024; // 64KB
	char* buffer = new char[SIZE];
	char* readback = new char[SIZE];

	// Fill buffer with a pattern: 'A', 'B', 'C', ..., cycling
	for (int i = 0; i < SIZE; ++i)
		buffer[i] = 'A' + (i % 26);

	int written = file.Write(SIZE, buffer);
	assert(written == SIZE);

	Console::puts("\nWritten bytes: ");
	Console::puti(written);
	Console::puts("\n\n");

	file.Reset();
	int read = file.Read(SIZE, readback);
	assert(read == SIZE);
	...
	for (int i = 0; i < SIZE; ++i) {
		assert(buffer[i] == readback[i]);
	}

	Console::puts("\n64KB file test passed.\n\n");

	delete[] buffer;
	delete[] readback;

	assert(_file_system->DeleteFile(42));
}
...
int main() {
	...
	#ifdef _USE_LARGE_FILE_TEST_
		test_large_file(FILE_SYSTEM);
	#else
		for (int j = 0; j < 30; j++) {
