...
_threads_low_switch_to:
	...
	; Make the new thread current
	mov	[_current_thread], eax
	
	; Check if the new thread has a page directory (process with separate address space)
	; page_directory is at offset 24 in Thread structure
	mov	ebx, [eax+24]	; Load page_directory pointer (PageDirectory*)
	cmp	ebx, 0		; Check if it's NULL
	je	.no_page_switch	; If NULL, skip page directory switch
	
	; Get the physical address of the page directory
	; PageDirectory structure: page_dir (PageDirectoryEntry*) at offset 0
	; page_dir contains the physical address of the page directory
	mov	ecx, [ebx]	; Load page_dir (physical address) from PageDirectory object
	
	; Load the page directory into CR3
	; This also flushes the TLB (Translation Lookaside Buffer)
	mov	cr3, ecx
...
.context_load_only:
	...
	mov	ebx, [eax+24]	; Load page_directory pointer (PageDirectory*)
	cmp	ebx, 0		; Check if it's NULL
	je	.no_page_switch_init	; If NULL, skip page directory switch
